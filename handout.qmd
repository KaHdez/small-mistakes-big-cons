...
-   R runs perfectly even if the data is wrong.
...
The previous mistakes look tiny, but later they affect totals, predictions, and business decisions. Let's take now a look at how these errors can be related to real life examples:
...
1. **Importing:** R doesn't convert text into categories automatically.
  + *Bad Practice:* `read.csv("file.csv")`
  + *Good Practice:* `read.csv("file.csv", stringsAsFactors = FALSE)`
  - *Business Example:* Customer IDs don’t match, therefore sales linked to wrong clients.

2. **Missing values:** Get real averages instead of NA results that look "valid" but are unusable.
  + *Bad Practice:* `mean(x)`
  + *Good Practice:* `mean(x, na.rm = TRUE)`
  - *Business Example:* Sales forecast too low that will lead to underproduction.

3. **Joining tables:** Avoid accidental duplicates and can see which rows didn't match.
  + *Bad Practice:* `merge(df1, df2)`
  + *Good Practice:* `left_join(df1, df2, by = "ID")` + `anti_join()`
  - *Business Example:* HR turnover looks higher, by consequence there will be unnecessary hiring.

4. **Data types:** Verify and enforce data types instead of hiping R guessed correctly. ***Only applicable for versions prior to R 4.0***
  + *Bad Practice:* Assuming R knows the type
  + *Good Practice:* `str(), glimpse(), mutate()`
  - *Business Example:* Revenue calculations wrong leading to bad budgeting.

5. **Checking data:** Catch mistakes before modeling instead of discovering them too late.
  + *Bad Practice:* Skipping inspection.
  + *Good Practice:* `summary(), head(), table(), view()`
  - *Business Example:* Management trusts a dashboard that is wrong.

6. **Filtering:** Keeps the dataset clean and consistent instead of returning unpredictable formats.
  + *Bad Practice:* `data[data$age > 18]`
  + *Good Practice:* `filter(data, age > 18)`
  - *Business Example:* Customer segments misclassified.


7. **Removing duplicates:** Removes duplicates but also lets us see exactly which rows were duplicated.
  + *Bad Practice:* `unique(data)`
  + *Good Practice:* `distinct(data, .keep_all = TRUE)`
  - *Business Example:* Marketing overspends on duplicated customers.

8. **Renaming columns:** Rename the correct column even if column order later changes.
  + *Bad Practice:* `colnames(data) [3] <- "sales"`
  + *Good Practice:* `rename(data, sales = old_name)`
  - *Business Example:* KPIs calculated using the wrong metric.

9. **Selecting columns:** Safer because column names don't change, column numbers do.
  + *Bad Practice:* `data[, c(1,2,5]`
  + *Good Practice:* `select(data, name, price, region)`
  - *Business Example:* Reports based on wrong variables.

10. **Dates:** Prevents R from guessing the wrong date format, which happens often.
  + *Bad Practice:* `as.Date(Date)`
  + *Good Practice:* `as.Date(date, format="%d/%m/%Y")` or `lubridate::dmy()`
  - *Business Example:* Sales trends analyzed for wrong time period.

11. **Reading Excel:** Makes sure to always read the correct sheet even if the sheet order changes.
  + *Bad Practice:* `read:excel("file.xlsx")`
  + *Good Practice:* `read_excel("file.xlsx", sheet="Sales2024)`
  - *Business Example:* Decisions based on outdated data.

12. **Plotting:** Control exactly what gets plotted instead of letting R "guess".
  + *Bad Practice:* `plot(data)`
  + *Good Practice:* `ggplot(data, aes(x, y))` + `geom_line()`
  - *Business Example:* Executives draw wrong conclusions from graphs.

13. **Pipelines (chaining steps):** A pipeline shows the full logic in one place, makes the order of steps clear, and makes the code easier to read, review, and debug.
  + *Bad Practice:* Many separate transformations that overwrite the dataset multiple times.
  + *Good Practice:* Use a single pipeline to apply all steps in order.
  - *Business Example:* Losing track of transformations.

14. **Saving:** Always keep the original data safe in case of errors.
  + *Bad Practice:* Overwriting the raw file
  + *Good Practice:* Save a separate clean file
  - *Business Example:* Errors cannot be traced or corrected later.
...
2.  R won't warn you when the result is wrong, only when the code is.
